...
// import Search from "../../shared/Search10";
...
function Division() {
  ...
  const handleAddDivision = () => {
    ...
    axios
      ...
      .then(async (res) => {
        ...
        setdivisions([res.data.doc, ...divisions]);
        await axios.post("/activitylog/create", {
          activity: `Division ${res.data.doc.name} created`,
          user: "admin",
        });
      })
      .catch((err) => {
  ...
  const addButton = (
    <button type="button" onClick={handleOpenAdd} className="btn green__btn btn__lg">
      Add New Division
    </button>
  );
  ...
  return (
    <div>
      <Search
        ...
        title="Search for a Division"
        inputFields={inputFields}
        addButton={addButton}  // Pass the button here
      />
